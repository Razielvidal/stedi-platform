...
export async function POST(req: NextRequest) {
    console.log('Line 4: POST request received for rapid step test');
    const token = req.headers.get('suresteps-session-token');
    console.log('Line 6: Session token:', token);
    ...
    const { customer, startTime, stepPoints, deviceId, totalSteps, stopTime, testTime } = data;
    console.log('Line 14: Request body to old API:', { customer, startTime, stepPoints, deviceId, totalSteps, stopTime, testTime });
    ...
    try {
        ...
        if (Array.isArray(stepPoints) && stepPoints.length > 0 && typeof stepPoints[0] === 'object') {
        ...
        console.log('Line 62: Old API response status:', response.status);
        console.log('Line 63: Old API response headers:', Object.fromEntries(response.headers.entries()));
        ...
        if (!response.ok) {
            if (contentType && contentType.includes('application/json')) {
                ...
                console.log('Line 71: Old API error JSON:', errorText);
            } else {
                responseText = await response.text();
                errorText = responseText;
                console.log('Line 75: Old API error text:', responseText);
            }
            console.error('Line 77: Rapid step test failed:', response.status, errorText);
            return NextResponse.json({ error: 'Failed to save step data', details: errorText }, { status: response.status });
        }

        // Log success response
        //responseText = await response.text();
        responseText = await response.text();
        console.log('Lin3 84: Old API success response text:', responseText);
        // Return "Saved" response as requested
        return new NextResponse('Saved', { status: 200 });
    } catch (error) {
        console.error('Line 88: Rapid step test error:', error);
