...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!stediResponse.ok) {
      return NextResponse.json({ error: result || 'Failed to create user on Stedi' }, { status: stediResponse.status });
    }
    ...
    return NextResponse.json({ error: 'Failed to connect to Stedi' }, { status: 500 });
  }
}
